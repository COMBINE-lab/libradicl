...
fn main() -> anyhow::Result<()> {
    ...
    let tag_context = p.get_record_context::<ScLongReadRecordContext>()?;
    let first_chunk = Chunk::<ScLongReadRecord>::from_bytes(&mut ifile, &tag_context);
    println!(
        "Chunk :: nbytes: {}, nrecs: {}",
    ...
    while total_chunks < p.hdr.num_chunks {
        let chunk = Chunk::<ScLongReadRecord>::from_bytes(&mut ifile, &tag_context);
